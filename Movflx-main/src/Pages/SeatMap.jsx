...
const SeatMap = () => {
 ...
 const { movieId, showDateId, scheduleId } = useParams();
  ...
  const getSeatStyles = (type, status, isSelected) => {
  ...
  const handleContinue = () => {
    ...
    // Giả sử bạn lấy các thông tin cần thiết từ props, state hoặc API
    const bookingInfo = {
      addScore: 10,
      bookingDate: new Date().toISOString().slice(0, 16),
      movieName: "The Great Movie", // lấy từ props hoặc API thực tế
      scheduleShow: "09:00", // lấy từ props hoặc API thực tế
      scheduleShowTime: "09:00", // lấy từ props hoặc API thực tế
      status: true,
      totalMoney: calculateTotalPrice(),
      useScore: 0,
      seat: selectedSeats.join(", ")
    };
    // Giả sử accountId lấy từ localStorage hoặc context
    ...
    <section className='results-sec'>
      <div className='container'>
        <div className='section-title'>
          <h5 className='sub-title'> Seat Map</h5>
          <h2 className='title'> Select Your Seats</h2>
          <p className='desc text-white'>Choose your preferred seats from the map below.</p>
        </div>
                    ...
                    <div className="flex">
                      {row.seats.map((seat) => {
                        ...
                        return (
                          <div
                            key={seat.seatCode}
                            className={getSeatStyles(seat.seatType, seat.status, isSelected)}
                          ...
                          >
                            <FaCouch
                              className={`text-2xl ${seat.status === "BOOKED" ? "text-white" : isSelected ? "text-white" : "text-gray-700"}`}
                            />
                            {/* Tooltip */}
